...
public class BasketResource {
    ...
    @DELETE
    @Path("/{basketId}/fruit/{fruitId}")
    public Response removeItem(
            @PathParam("basketId") Long basketId,
            @PathParam("fruitId") Long fruitId) {

        boolean removed = basketService.removeItemFromBasket(basketId, fruitId);

        if (!removed) {
            return Response.status(Response.Status.NOT_FOUND)
                    .entity("Item not found in this basket")
                    .build();
        }

        return Response.ok("Item removed" + fruitId.toString()).build();
    }
